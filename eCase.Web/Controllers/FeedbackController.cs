...
using eCase.Web.Models.Feedback;
...
namespace eCase.Web.Controllers
{
    public partial class FeedbackController : BaseController
    {
        ...
        public virtual ActionResult Index(FeedbackVM vm, bool? captchaValid)
        {
            ...
            vm.Name += (CurrentUser != null && !string.IsNullOrEmpty(CurrentUser.Email)) ? string.Format(" потребител: {0}", CurrentUser.Email) : string.Empty;

            using (var transaction = _unitOfWork.BeginTransaction())
            {
                foreach (var recipient in ConfigurationManager.AppSettings["eCase.Web:FeedbackEmail"].Split(','))
                {
