...
function gridViewLoadData(page) {
    ...
    $.ajax({
        ...
        success: function (result) {
            console.log(result);
            //debugger;
            if (result.data && result.data.length == 0 && gridViewState.empty_text) {
...
function gridViewLoadData_templateSizeSelector(selectedSize) {
    ...
    for (var i = 0; i < sizes.length; i++) {
        ...
        result += '<a href="#" onclick="gridViewSetSize(' + sizes[i] + ');return false;"' + selected + '>' + allText + '</a>';
    }
    result += '</div>';
    return result;
}
...
$('#gridview').gridView({
    url: '@(Url.Action("GetFileList","FileCdn"))',
    method: 'POST',
    data: {
