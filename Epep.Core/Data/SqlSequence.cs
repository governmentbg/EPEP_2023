...
{
    public class SqlSequenceSimple
    {
        private string sequenceName;
        private string connectionStringName;
        public SqlSequenceSimple(string sequenceName, string connectionStringName)
        {
            this.sequenceName = sequenceName;
            this.connectionStringName = connectionStringName;
        }

        public async Task<long> NextValue()
        {
            long result = -1;
            using (SqlConnection connection = new SqlConnection(connectionStringName))
            {
                ...
                try
                {
                    using (SqlCommand cmdNextVal = new SqlCommand($"Select (NEXT VALUE FOR {sequenceName}) as val", connection))
                    {
                        var reader = await cmdNextVal.ExecuteReaderAsync();
                        while (reader.Read())
                        {
                            var txtValue = reader.GetSqlValue(0).ToString();
                            result = long.Parse(txtValue);
                        }
                    }
                }
                catch (Exception ex)
    ...
    public class SqlSequence
    {
        ...
        public SqlSequence(string sequenceName, string connectionStringName)
        {
            this.sequenceName = sequenceName;
            this.connectionStringName = connectionStringName;
            this.lastValue = 0;
